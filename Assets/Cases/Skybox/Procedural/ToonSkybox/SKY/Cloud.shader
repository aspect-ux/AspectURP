Shader "Unlit URP Shader Cloud"
{
    ...
    SubShader
    {
        ...
        // 先渲染不透明物体，然后由远及近绘制半透明物体，半透需要关闭ZWrite防止半透遮挡不透，同时深度测试要开启
        ZWrite Off
        Cull Off
        ZTEST Less
        //ZTest NotEqual
        Pass
        {
            ...
            half4 frag(Varyings i) : SV_Target
            {
               ...
               float4 Noise = SAMPLE_TEXTURE2D(_NoiseMap,sampler_NoiseMap,i.noiseuv);
                 
               //float4 baseMap = tex2D(_CloudMap, i.uv);
           
               float UVdisturbance =  remap(Noise.b,0,1,0,0.03);

               float4 baseMap = SAMPLE_TEXTURE2D(_CloudMap,sampler_CloudMap, i.uv + UVdisturbance);
