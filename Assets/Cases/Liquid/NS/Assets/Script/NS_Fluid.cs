...
{       
   ...
   public Texture2D oriTex;//原始纹理
    ...
    private RenderTexture vel_0, vel_1, col_0, col_1, div_0, layIn_vel, layIn_ve2, col_00, col_11;
	...
	private void Start() 
    {
		...
		col_00 = new RenderTexture(texRes, texRes, 0, RenderTextureFormat.ARGBHalf);
		col_1 = new RenderTexture(texRes, texRes, 0, RenderTextureFormat.ARGBHalf);
		col_11 = new RenderTexture(texRes, texRes, 0, RenderTextureFormat.ARGBHalf);

		div_0 = new RenderTexture(texRes, texRes, 0, RenderTextureFormat.RHalf);

        layIn_vel = new RenderTexture(texRes, texRes, 0, RenderTextureFormat.ARGBFloat);
        layIn_ve2 = new RenderTexture(texRes, texRes, 0, RenderTextureFormat.ARGBFloat);
		Graphics.Blit(oriTex, col_0);
		Graphics.Blit(oriTex, col_00);
    ...
    private void SimilateFluid()
    {
        ...
        simMat.SetTexture("_Tex0", vel_0);
        simMat.SetTexture("_Tex1", layIn_vel);
		simMat.SetFloat("advectSpeed", advectSpeed);
		Graphics.Blit(null, layIn_ve2, simMat, 6);//render
		SwapTexture(ref layIn_vel, ref layIn_ve2);//->0

		simMat.SetTexture("_Tex0", layIn_vel);
		simMat.SetTexture("_Tex1", col_00);
		Graphics.Blit(null, col_11, simMat, 7);//render

		if (Input.GetKeyDown(KeyCode.A))//还原
        {
            isReset = true;
        ...
        {
			simMat.SetTexture("_Tex0", layIn_vel);
			Graphics.Blit(null, layIn_ve2, simMat, 8);//render
			SwapTexture(ref layIn_vel, ref layIn_ve2);//->0
		}
		if (rawImage)
		{
			rawImage.texture = layIn_vel;
		}
        if (rawImage1)
        {
            rawImage1.texture = col_11;
